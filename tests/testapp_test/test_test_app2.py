...
from testapp.command import ReadCommand, WriteCommand
...
class TestTestApp2(TestCase):
    ...
    @print_function_name
    @patch.object(WriteCommand, 'run', return_value=True)
    ...
    @print_function_name
    @patch.object(ReadCommand, 'run', return_value=READ_VALUE)
    ...
    @print_function_name
    @patch.object(WriteCommand, 'run', return_value=True)
    @patch.object(ReadCommand, 'run', return_value=f'0x{READ_VALUE:02x}')
    def test_run_SHOULD_return_True_When_normal(self, read_mock, write_mock):
        self.assertTrue(self.test_app2.run())

    @print_function_name
    @patch.object(WriteCommand, 'run', return_value=True)
    @patch.object(ReadCommand, 'run', return_value=f'0x{WRITE_VALUE:02x}')
